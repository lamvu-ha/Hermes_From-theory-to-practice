...
flowchart TD
    ...
    task([👤 Người dùng giao nhiệm vụ]) -->|bắt đầu phiên| read_rules[Đọc luật dự án]
    read_rules -->|đã biết luật| inspect_repo[Đọc cấu trúc repo]
    inspect_repo -->|đã biết repo có gì| todo[Lập TODO ngắn]
    todo -->|theo checklist| inspect_existing[Kiểm tra file hiện có]
    inspect_existing -->|đã chọn phạm vi sửa| edit_files[Tạo hoặc sửa file]
    edit_files -->|sau mỗi cụm thay đổi| run_checks[Chạy kiểm tra thật]
    run_checks -->|đọc kết quả terminal| has_error{Có lỗi?}
    has_error -->|Có lỗi| read_error[Đọc error log]
    read_error -->|đã khoanh vùng| diagnose[Phân tích nguyên nhân]
    diagnose -->|có hướng sửa| patch_file[Sửa file liên quan]
    patch_file -->|kiểm chứng lại| run_checks
    has_error -->|Không lỗi| summarize[Tóm tắt thay đổi]
    summarize -->|sau khi hoàn thành| reusable{Quy trình tái sử dụng được?}
    reusable -->|Có| save_learning[Lưu bài học]
...
Sơ đồ chỉ giữ tên bước ngắn để dễ nhìn. Chi tiết bên trong từng ô được giải thích ở bảng dưới: mỗi bước có nguồn dữ liệu, việc Hermes làm, và đầu ra để chuyển sang bước tiếp theo. Ví dụ, ô `Đọc luật dự án` không chỉ là "đọc file"; nó lấy `SOUL.md` để biết phong cách/kỷ luật agent, lấy `AGENTS.md` để biết luật repo, rồi tạo ra bộ ràng buộc như stack, lệnh kiểm tra, vùng cấm xóa file hoặc push/deploy.
...
| Ô trong luồng | Nguồn dữ liệu | Bên trong ô diễn ra gì | Đầu ra / điều kiện đi tiếp |
| --- | --- | --- | --- |
| `Đọc luật dự án` | `SOUL.md`, `AGENTS.md` | Load persona/rules, xác định tech stack, cách trả lời, lệnh kiểm tra, vùng cấm | Có đủ ràng buộc để không làm sai repo |
| `Đọc cấu trúc repo` | File tree, `README.md`, thư mục backend/frontend | Xem repo đã có gì, backend/frontend đã scaffold chưa | Biết nên tạo mới hay sửa trên nền có sẵn |
| `Lập TODO ngắn` | Prompt người dùng + luật repo | Chia MVP thành các việc nhỏ: API, DB, UI, build/test | Có checklist đủ để triển khai không bỏ sót |
| `Kiểm tra file hiện có` | Các file liên quan trong repo | Đọc code trước khi sửa để tránh ghi đè sai | Xác định file cần tạo/sửa |
| `Tạo hoặc sửa file` | TODO + file hiện có | Viết code theo từng cụm nhỏ, tránh rewrite toàn bộ | Một cụm thay đổi có thể kiểm tra được |
| `Chạy kiểm tra thật` | Terminal output, test/build log | Chạy terminal/test/build/import check và đọc output | Nếu lỗi thì vào nhánh debug, nếu pass thì tóm tắt |
| `Đọc error log` | Stack trace, dòng lỗi, file lỗi | Khoanh vùng nơi hỏng và triệu chứng chính | Có manh mối để chẩn đoán |
| `Phân tích nguyên nhân` | Error log + source code | Xác định lỗi do import, schema, env, dependency hay logic | Có hướng sửa cụ thể |
| `Sửa file liên quan` | Nguyên nhân đã xác định | Patch đúng file, giữ thay đổi nhỏ và đúng lỗi | Quay lại chạy kiểm tra |
| `Lưu bài học` | Kết quả đã pass + bài học rút ra | Ghi `MEMORY.md` hoặc tạo draft `SKILL.md` khi quy trình tái sử dụng được | Kết thúc với kinh nghiệm đã lưu |
