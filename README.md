...
```mermaid
%%{init: {"flowchart": {"nodeSpacing": 34, "rankSpacing": 46}, "themeVariables": {"fontSize": "17px"}}}%%
flowchart TB
    ...
    subgraph verify["4. Kiểm tra bằng môi trường thật"]
        ...
        v2[Chạy backend test/run]
        v3[Chạy frontend build]
        v4{Có lỗi?}
        v1 --> v2 --> v3 --> v4
    end

    subgraph learn["5. Tổng kết và học lại"]
        direction TB
    ...
    subgraph debug["Nhánh phụ: Debug nếu thất bại"]
        direction TB
        d1[Đọc error log]
...
flowchart TD
    ...
    fix_request -->|bắt đầu| rc1
    rc3 -->|đã biết lệnh| rp1
    rp3 -->|có log| dg1
    dg3 -->|có giả thuyết| i1
    i3 -->|biết chỗ sửa| p1
    p3 -->|kiểm tra lại| vf1
    vf2 -->|Có| rp2
    ...
    class vf2 decision
    class rc1,rc2,rc3,rp1,rp2,rp3,dg1,dg2,dg3,i1,i2,i3,p1,p2,p3,vf1 process
    class fix_request,vf3 success
```
